...
## 2026-07-25 - [Sync CSP headers between netlify.toml and index.html]
...
## 2026-07-12 - [Client-Side IDOR Defense]
**Vulnerability:** The `getTodayLogs` function in `src/lib/firestore.ts` lacked authorization checks when retrieving unsynced local logs from `localStorage`. This allowed a user to retrieve and display habit logs generated by a previous user on the same device (Cross-User Data Contamination / Client-Side IDOR).
**Learning:** For client-side storage mechanisms like `localStorage`, data representing user-specific context must be rigorously filtered to ensure it belongs to the currently active session, as the storage is persistent and shared across all sessions on the device.
**Prevention:** Always enforce ownership by filtering retrieved local objects (e.g., `filter(log => log.user_id === userId)`) when combining cached local data with authenticated remote data.
## 2026-07-27 - [Client-Side IDOR Defense in Cached Habits]
**Vulnerability:** The Dashboard component loaded and displayed all locally cached habits from `localStorage` without verifying that they belonged to the currently authenticated user. This allowed one user to briefly see another user's habits on the initial render if they shared a device (Cross-User Data Contamination / Client-Side IDOR).
**Learning:** For client-side storage mechanisms like `localStorage`, data representing user-specific context must be rigorously filtered to ensure it belongs to the currently active session, as the storage is persistent and shared across all sessions on the device.
**Prevention:** Always enforce ownership by filtering retrieved local objects (e.g., `filter(habit => habit.user_id === currentUser)`) when restoring state from `localStorage` or other local caches.
## 2026-07-28 - [Missing Payload Validation in Firestore Mutations]
**Vulnerability:** The data access layer (e.g., `src/lib/firestore.ts`) was executing Firestore mutations (such as `saveUserSettings` and `logHabitCompletion`) without strictly validating the types and boundaries of the incoming payload, particularly for fields like `settings`, `date`, and `completed`. This allowed the potential injection of malformed or excessively large data objects, leading to application-level Denial of Service when corrupted data is rendered or processed by clients.
**Learning:** Even when the frontend UI is trusted, the data access layer must act as a distinct security boundary. Client-side storage and NoSQL document updates must enforce strict schema and type validation at runtime to prevent malformed data from propagating to the backend and contaminating the data model.
**Prevention:** Always implement rigorous runtime type checking and boundary validation for all arguments passed to data mutation functions (e.g., checking that `settings` is an object with valid properties, and `date` is a valid `Date` object) before executing Firestore operations.
## 2026-07-29 - [Missing Payload Validation in addHabit]
**Vulnerability:** The `addHabit` function in `src/lib/firestore.ts` was executing Firestore mutations without strictly validating the types and presence of the incoming payload, particularly for fields like `name` and `category`. This allowed the potential injection of malformed data objects (e.g. replacing a string with an object), leading to application-level Denial of Service when corrupted data is rendered or processed by clients.
**Learning:** The data access layer must enforce strict schema, presence, and type validation at runtime for all object properties sent to the database to prevent malformed data from propagating to the backend and contaminating the data model.
**Prevention:** Always implement rigorous runtime type checking and boundary validation for all arguments passed to data mutation functions (e.g., checking that `name` is a valid string, and `category` is a valid string) before executing Firestore operations.
## 2026-07-31 - [Client-Side DoS via Unhandled JSON.parse on LocalStorage]
**Vulnerability:** The application was calling `JSON.parse()` directly on data retrieved from `localStorage` in functions like `loadHabits`, `loadChecks`, and when filtering `unsyncedLogs`. If the stored data was corrupted or maliciously malformed (e.g., via a different origin or previous vulnerable state), it would throw a `SyntaxError`, causing a persistent client-side Denial of Service because the unhandled exception crashed the app or prevented data loading.
**Learning:** Client-side storage (`localStorage`, `sessionStorage`, `IndexedDB`) is untrusted input. Attempting to deserialize it without error handling creates a fragile application state where corrupted data bricks the application until manually cleared by the user.
**Prevention:** Always wrap `JSON.parse()` calls reading from client-side storage in a `try...catch` block. If parsing fails, fail securely by returning a default empty state (e.g., `[]` or `null`) and optionally logging the error internally.
## 2026-08-09 - [Property Injection/Stripping in Firestore Mutations]
**Vulnerability:** The `saveUserSettings` function in `src/lib/firestore.ts` validated the expected properties of the `settings` object but passed the original untrusted object directly to the Firestore `setDoc` function. This allowed an attacker to inject arbitrary, unvalidated properties into the database (e.g., passing `{ theme: 'light', isAdmin: true }`), contaminating the user document and potentially causing security issues elsewhere.
**Learning:** Checking the types and values of expected object properties is insufficient if the untrusted object itself is passed to the database. The database driver will serialize and save any additional properties on the object that were not checked.
**Prevention:** Always explicitly construct a new payload object containing only the expected, validated properties before passing data to Firestore mutations, thereby stripping out any unvalidated or malicious extra fields.
## 2026-08-14 - [Persistent DoS via Invalid Theme Color Injection]
**Vulnerability:** The `saveUserSettings` function in `src/lib/firestore.ts` validated the type of `settings.themeColor` but did not verify if the string belonged to the allowed set of theme colors. This allowed an attacker or corrupted state to inject arbitrary strings (e.g., `invalid`) which were subsequently used to index the `themes` object in `mui-theme.ts`. If the key was missing, accessing `.color` on the undefined object would throw a TypeError, causing a persistent client-side Denial of Service because the app would crash continuously upon load.
**Learning:** Checking the basic type of a configuration string is insufficient if that string is later used as a direct index to an object or dictionary. The write boundary must ensure strict allowlisting, and the read boundary must employ defensive defaults.
**Prevention:** Always validate configuration strings against an explicitly defined allowlist before writing to the database or storage. Additionally, implement safe default fallbacks (e.g., `themes[themeColor] ? themeColor : 'red'`) when dynamically indexing into objects to gracefully handle corrupted or missing keys.
## 2026-08-19 - [Client-Side DoS via Unhandled JSON.parse Type Confusion]
